...
export class LoginPageSec {
  ...
  async expectLogout() {
    await expect(
      this.page.getByRole("heading", { name: "Account Logout" }),
    ).toBeVisible();
    await expect(
      this.page.getByText("You have been logged off your"),
    ).toBeVisible();
    await expect(
      this.page.getByText("Your shopping cart has been"),
    ).toBeVisible();
  }
